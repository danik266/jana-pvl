...
import { 
  ...
  ShieldCheck, AlertCircle, Fingerprint, Sparkles 
} from "lucide-react";
import Link from "next/link";

// 2. НАСТРАИВАЕМ ШРИФТ (поддержка кириллицы обязательна)
...
export default function AuthPage() {
    ...
    try {
      if (isLogin) {
        ...
        if (data.session) {
            ...
            alert("Тіркелу сәтті өтті! Почтаңызды тексеріңіз. / Регистрация успешна! Проверьте почту.");
            setIsLogin(true);
        }
      }
    } catch (error) {
  ...
  return (
    // 3. ПРИМЕНЯЕМ КЛАСС ШРИФТА КО ВСЕМУ КОНТЕЙНЕРУ (montserrat.className)
    <div className={`${montserrat.className} min-h-screen flex items-center justify-center bg-slate-50 relative overflow-hidden p-4`}>
      ...
      <Link href="/" className="absolute top-6 left-6 group flex items-center gap-2 text-gray-500 hover:text-cyan-600 transition-colors z-10">
        ...
        <div className="bg-white rounded-3xl shadow-2xl shadow-cyan-900/10 border border-white/60 overflow-hidden relative">
          ...
          <div className="p-8 pb-2 text-center">
            ...
            <p className="text-gray-400 text-sm mb-6 font-medium">
              {isLogin ? "Smart Pavlodar Tourism Platform" : "Создайте аккаунт путешественника"}
            </p>
          </div>
          ...
          <div className="px-8 pb-8">
            ...
            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); handleAuth(); }}>
              ...
              <div className={`transition-all duration-300 overflow-hidden ${isLogin ? 'h-0 opacity-0' : 'h-[80px] opacity-100'}`}>
                <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5 ml-1">Аты-жөніңіз / ФИО</label>
                <div className="relative group">
                  <User className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-cyan-500 transition-colors" size={20} />
                  <input
                    type="text"
                    required={!isLogin}
                    value={fullName}
              ...
              <div>
                ...
                <div className="relative group">
                  <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-cyan-500 transition-colors" size={20} />
                  <input
                    type="email"
                    required
                    value={email}
              ...
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5 ml-1">Құпия сөз / Пароль</label>
                <div className="relative group">
                  <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-cyan-500 transition-colors" size={20} />
                  <input
              ...
              <div className="pt-2">
                ...
                >
                    ...
                    <span className="relative flex items-center justify-center gap-2">
                        {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"/> : (isLogin ? "Кіру / Войти" : "Тіркелу / Создать")}
            ...
            <div className="relative my-8">
              ...
              <div className="relative flex justify-center text-xs font-bold uppercase tracking-widest"><span className="px-3 bg-white text-gray-300">немесе / или</span></div>
            </div>
            ...
            <div className="mt-8 text-center">
              ...
              <button onClick={toggleMode} className="text-cyan-600 hover:text-teal-600 font-bold transition-colors text-sm hover:underline decoration-2 underline-offset-4">
                {isLogin ? "Тіркелу (Зарегистрироваться)" : "Кіру (Войти в аккаунт)"}
              </button>
            </div>
          </div>
        </div>
